...
export default class ActionsDisplayAnims {
  ...
  bindToDOM() {
    ...
    this.containerBtnsActions.append(
      this.btnActionColapse,
      this.btnActionCallbackChat,
      this.btnActionLiker,
    );

    this.elParent.appendChild(this.containerClassesBlocks);

    this.containerBtnsActions.addEventListener(
      "click",
  ...
  onClick(e) {
    Object.keys(this.classes).forEach((classEl) => {
      ...
      if (
        e.target.classList.contains(`btn-action-${this.classes[classEl].name}`)
      ) {
        if (
          this.containerClassesBlocks.querySelector(
            `.${this.classes[classEl].name}-container`,
          )
        ) {
          this.containerClassesBlocks
            .querySelector(`.${this.classes[classEl].name}-container`)
            .remove();
        } else {
          if (this.classes[classEl].anims) {
            const instance = new this.classes[classEl].class(
              this.containerClassesBlocks,
              this.classes[classEl].anims,
            );
            instance.bindToDOM();
          } else {
            const instance = new this.classes[classEl].class(
