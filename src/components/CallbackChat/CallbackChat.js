...
export default class CallbackChat {
    ...
    bindToDOM() {
        this.callbackChatContainer = new Div({ class: 'callback-chat-container' }).element;
        this.btnAddChat = new Button({ class: 'btn-add-chat', type: 'button' }).element;

        this.form = new Form({ class: 'form-container' }).element;
        this.formTitleCloseBtn = new Div({ class: 'form-title-close-btn' }).element;
        this.formTitle = new Heading({ class: 'form-title', text: 'Напишите нам', level: 1 }).element;
        this.formTextarea = new Textarea({ class: 'form-textarea', name: 'message', text: '' }).element;
        this.closeBtn = new Button({ class: 'close-btn', type: 'button', text: 'x' }).element;
        this.submitBtn = new Button({ class: 'submit-btn', type: 'button', text: 'Отправить' }).element;
